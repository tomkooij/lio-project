...
#
HIGH_PH = 200		# ADC counts
LOW_PH = 120		# ADC counts
CUT_OFF_PH = 0		# the minimum registred PH is 20 ADC counts, but we check fot t != -999 
...
def open_event_file(filename, stations, start, end):

    print   "creating or opening file: ",filename
    data = tables.open_file(filename,'a')
	
    if '/s%d' % stations[0] in data:
...
for month in range(1,13):
    startdatum = datetime.datetime(2014,month,1)
    if month < 12:    
        einddatum = datetime.datetime(2014,(month+1),1)
    else:
