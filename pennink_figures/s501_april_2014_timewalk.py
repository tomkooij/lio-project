...
if __name__=='__main__':
    ...
    dt_t2hoog_t2hoog = (t1-t2).compress( (t1 >0) & (t2 > 0) & (ph1 > HIGH_PH) & (ph2 > HIGH_PH))
    ...
    dt_t2laag_t2laag = (t1-t2).compress( (t1 >0) & (t2 > 0) & (ph1 > 20.) & (ph2 > 20.) & (ph1 < LOW_PH) & (ph2 < LOW_PH))
    ...
    if 0:
        ...
        c, fitx, fity = gauss_fit_histogram(n1, bins1, sigma=sigma_list, initialguess = [100.,10., 10., 0.], verbose=True)
        mu = c[1]
        sigma = abs(c[2])
    ...
    print "ph2 laag, ph2 laag", dt_t2hoog_t2hoog.size
    n1, bins1, blaat1 = plt.hist(dt_t2hoog_t2hoog, bins=bins2ns5, histtype='step')
    ...
    t_walk = lambda x: -3. + (22.3 / (x-20.)**0.156)

    t1_corr = t1 - t_walk(ph1)
    t2_corr = t2 - 20 #- t_walk(ph2)
    dt_corr = (t1_corr-t2_corr).compress( (t1 >0) & (t2 > 0) & (ph1 > 20.) & (ph2 > 20.) & (ph1 < LOW_PH) & (ph2 < LOW_PH))
